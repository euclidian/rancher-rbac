<template>
  <v-app>
    ...
    <v-card v-if="dialogService == true && dialogFormService == false">
      <v-btn color="white" class="mb-0 black--text" fab small @click="dialogService =false">
        <v-icon dark>close</v-icon>
      </v-btn>
      <v-card-title>
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn
                  ...
                  small
                  color="blue"
                  @click="showEnvironments(props.item.id)"
                  v-on="on"
                >
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn
                  ...
                  small
                  color="warning"
                  @click="formEditService(props.item.id)"
                  v-on="on"
                >
    ...
    <v-card v-if="editStack == true && dialogService == false && dialogFormService == false">
      ...
      <v-card-text>
        <v-container grid-list-md>
          <v-layout wrap>
            <v-flex xs12>
              <v-text-field v-model="rancherStackId" :counter="10" label="Name" required readonly></v-text-field>
              <v-textarea v-model="remarkStack" name="input-7-1" label="Remark" hint="Hint text"></v-textarea>
            </v-flex>
          </v-layout>
        </v-container>
      </v-card-text>
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn
                  ...
                  small
                  color="success"
                  @click="detailServiceStack(props.item.id)"
                  v-on="on"
                >
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn
                  ...
                  small
                  color="warning"
                  @click="formEditStack(props.item.rancher_stack_id)"
                  v-on="on"
                >
                  <v-icon dark>edit</v-icon>
                </v-btn>
            ...
            <v-tooltip top>
              <template v-slot:activator="{ on }">
                <v-btn
                  ...
                  small
                  color="error"
                  @click="deleteStack(props.item.rancher_stack_id)"
                  v-on="on"
                >
                  <v-icon dark>delete</v-icon>
                </v-btn>
...
export default {
  components: {
    "rancher-environment": EnvironmentComponent
  },
